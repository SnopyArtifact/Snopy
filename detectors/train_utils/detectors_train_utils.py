...
class IVDetectTrainUtil(BaseTrainUtil):
    ...
    def generate_features(self, sample: Dict[str, Union[str, List[int], List[str]]]) \
            -> Tuple[List[torch.Tensor], List[Tuple], List[torch.Tensor], List[torch.Tensor], List[
                torch.Tensor], torch.LongTensor, int]:
        return self.ivdetect_util.generate_all_features(sample)

    def generate_graph_from_feature(self, feature: Tuple[
        List[torch.Tensor], List[Tuple], List[torch.Tensor], List[torch.Tensor],
