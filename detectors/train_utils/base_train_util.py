...
class BaseTrainUtil:
    ...
    def evaluate(self, eval_dataloader, num_batch):
        ...
        with torch.no_grad():
            for batch_features in tqdm(eval_dataloader, total=num_batch, desc="evaluating process", file=sys.stdout):
                ...
                logits, _ = self.gnnNets(data=batch)
    ...
    def test(self):
        ...
        with torch.no_grad():
            for batch_features in tqdm(test_dataloader, total=num_batch, desc="testing process",
                ...
                batch: Batch = Batch.from_data_list(batch_data)
                batch.to(self.device)
                probs, feature_emb = self.gnnNets(data=batch)
                # if index == 1:
                #     feature_array = np.empty(feature_emb.shape, dtype=feature_emb.dtype)
                #     index += 1
                ...
                embeddings.extend(feature_emb.cpu().tolist())
                # torch.cat((feature_array, feature_emb), dim=0)
    ...
    def retrieval(self):
        ...
        train_vul_features = [self.generate_features(sample) for sample in
                              tqdm(self.train_positive, desc="generating feature for train_vul sample",
                                   file=sys.stdout)]
        val_vul_features = [self.generate_features(sample) for sample in
                            tqdm(self.val_positive, desc="generating feature for val_vul sample", file=sys.stdout)]
        end_time = time()
        spent_time = end_time - start_time
        print(f"embedding spent time: {spent_time // 3600}h-{(spent_time % 3600) // 60}m-{(spent_time % 3600) % 60}s")
        ...
        retrieval_dataloader = get_dataloader(train_vul_features, val_vul_features, self.train_args.batch_size)
        self.gnnNets.eval()
        print('start similar cluster retrieval==================')
        retrieval_start_time = time()
        num_batch = len(train_vul_features + val_vul_features) // self.train_args.batch_size
        with torch.no_grad():
            for batch_features in tqdm(retrieval_dataloader, total=num_batch, desc="testing process",
                                       file=sys.stdout):
                batch_data: List[Data] = [self.generate_graph_from_feature(feature, self.device) for feature in
                                          batch_features]
                batch: Batch = Batch.from_data_list(batch_data)
                batch.to(self.device)
                _, feature_emb = self.gnnNets(data=batch)
                embeddings.extend(feature_emb.cpu().tolist())

        df = pd.DataFrame(embeddings)
        df.to_csv('embeddings1.csv', index=False, header=False)

        retrieval_end_time = time()
        retrieval_spent_time = retrieval_end_time - retrieval_start_time
        print(
            f"retrieval spent time: {retrieval_spent_time // 3600}h-{(retrieval_spent_time % 3600) // 60}m-{(retrieval_spent_time % 3600) % 60}s")
    ...
    def train(self):
        ...
        optimizer = Adam(self.gnnNets.parameters(), lr=self.train_args.learning_rate,
