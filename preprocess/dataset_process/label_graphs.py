...
def process_func_datas(cpg_graphs: List[Dict[str, Union[str, List[str], List[int]]]]):
    ...
    for cpg_graph in tqdm(cpg_graphs, desc="labeling function graphs"):
        ...
        if testcase_path.split('_')[-1] == '0.c':
    ...
    for cpg_graph in tqdm(vul_samples, desc="removing noise vul samples"):
        md5_value = get_func_md5(cpg_graph)
        if md5_value in both_md5_set:
            continue
        final_vul_samples.append(cpg_graph)

    print(f"labeling graphs done. vulnerable function num: {len(final_vul_samples)}, "
          f"normal function num: {len(final_normal_samples)}")
    return vul_samples, final_normal_samples


def process_stmt_datas(label_info: Dict[str, List[int]],
    ...
    # 标注每一个graph
    for cpg_graph in tqdm(cpg_graphs, desc="labeling function graphs"):
        testcase_path = cpg_graph["testcase-path"].split('/')[-1]
        md5_value = get_func_md5(cpg_graph)
...
def dump_to_output_dir(vul_samples: List[Dict], normal_samples: List[Dict], output_json_dir: str):
    ...
    output_datas: List[List[Dict]] = [
        ...
        normal_samples[:train_normal_idx], normal_samples[train_normal_idx: test_normal_idx],
        normal_samples[test_normal_idx:]
    ]
...
if __name__ == '__main__':
    ...
    label_file = dataset_dir + 'label.json'
    print(label_file)
    output_json_dir = os.path.dirname(os.getcwd()) + '/graph_gen/'
    json_data_file = output_json_dir + sys.argv[2]
    print(json_data_file)
    output_dir = dataset_dir + sys.argv[3]
    print(output_dir)

    if level == "function":
        cpg_graphs: List[Dict[str, Union[str, List[str], List[int]]]] = \
            json.load(open(json_data_file, 'r', encoding='utf-8'))
        vul_samples, normal_samples = process_func_datas(cpg_graphs)
        ...
        cpg_graphs: List[Dict[str, Union[str, List[str], List[int]]]] = \
            json.load(open(json_data_file, 'r', encoding='utf-8'))
        vul_samples, normal_samples = process_stmt_datas(label_info, cpg_graphs)
        dump_to_output_dir(vul_samples, normal_samples, output_dir)
